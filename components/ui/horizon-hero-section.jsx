...
export const HorizonHeroSection = () => {
  ...
  const totalSections = 4; // Corresponding roughly to the page sections
  ...
  useEffect(() => {
    ...
    const initThree = () => {
      ...
      refs.scene.fog = new THREE.FogExp2(0x030303, 0.00025);
    ...
    const createStarField = () => {
      const { current: refs } = threeRefs;
      const starCount = 3000; // Optimized
      
      for (let i = 0; i < 2; i++) { // Optimized passes
        ...
        for (let j = 0; j < starCount; j++) {
          ...
          // Color variation (Monochrome / Sleek)
          const color = new THREE.Color();
          color.setHSL(0, 0, 0.3 + Math.random() * 0.7);
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          time: { value: 0 },
          color1: { value: new THREE.Color(0x111111) }, // Subtle dark
          color2: { value: new THREE.Color(0x333333) }, // Sleek grey
          opacity: { value: 0.15 } // Less opaque for cleaner background
        },
        vertexShader: `
          varying vec2 vUv;
    ...
    const createMountains = () => {
      ...
      const layers = [
        ...
        { distance: -100, height: 80, color: 0x0a0a0a, opacity: 0.9 },
        { distance: -150, height: 100, color: 0x111111, opacity: 0.7 },
        { distance: -200, height: 120, color: 0x1a1a1a, opacity: 0.5 }
      ];
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec3 atmosphere = vec3(0.2, 0.2, 0.2) * intensity;
      ...
      if (refs.mountains && refs.nebula) {
         refs.mountains.forEach((mountain, i) => {
            ...
            if (progress > 0.7) {
              ...
              if (refs.locations) mountain.position.z = refs.locations[i] || targetZ;
            }
         });
  ...
  return (
    <>
      <div ref={containerRef} className="relative w-screen h-screen overflow-hidden bg-[#030303] text-white flex items-center justify-center">
        {/* Background Canvas pinned fixed so it follows the scroll across the page */}
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full px-4 pt-16">
          ...
          <h1 ref={titleRef} className="text-5xl md:text-8xl lg:text-9xl font-extrabold tracking-tighter text-white pb-4 overflow-hidden flex justify-center" style={{ visibility: 'hidden' }}>
            {mounted ? splitTitle(titleText) : null}
          </h1>
          
          <div ref={subtitleRef} className="mt-4 flex flex-col items-center text-center space-y-4" style={{ visibility: 'hidden' }}>
            <p className="subtitle-line mt-2 text-base font-medium uppercase tracking-[0.2em] text-zinc-400 md:text-xl">
              Full-Stack Developer <span className="mx-1 text-white/10">&amp;</span> Automation Architect
            </p>
            <p className="subtitle-line max-w-2xl text-base font-light leading-relaxed text-zinc-300 md:text-lg">
              Building high-performance digital experiences and intelligent automation. 
              Currently focused on scaling <span className="text-white">full-stack architectures</span> and exploring the intersection of <span className="text-white">efficiency and design</span>.
            </p>

            {/* Actions aligned with original page aesthetic */}
            <div className="subtitle-line mt-12 flex flex-col items-center justify-center gap-6 sm:flex-row">
                ...
                <div className="ml-0 flex gap-3 sm:ml-2">
                  {[
                    ...
                    { icon: <Linkedin size={19} />, href: "https://www.linkedin.com/in/naman-soni-6977952b1/" },
                  ].map((social, index) => (
                    <a
                      key={index}
                      href={social.href}
                      ...
                      className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-zinc-400 transition-all hover:text-white hover:-translate-y-1 hover:bg-white/10"
                    >
                      {social.icon}
                    </a>
                  ))}
